...
void (*set_cluster_enabled_cb_cal)(int, int) = NULL;
...
void register_set_cluster_enabled_cb(void (*func)(int, int))
{
	...
	BUG_ON(set_cluster_enabled_cb_cal);
	set_cluster_enabled_cb_cal = func;
}
...
int cal_cluster_enable(unsigned int cluster)
{
	...
	if (likely(ret == 0 && set_cluster_enabled_cb_cal))
		set_cluster_enabled_cb_cal(cluster, 1);
...
int cal_cluster_disable(unsigned int cluster)
{
	...
	if (likely(ret == 0 && set_cluster_enabled_cb_cal))
		set_cluster_enabled_cb_cal(cluster, 0);
